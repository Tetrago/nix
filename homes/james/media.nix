  ...
  xdg = let
    desktopEntries = {
      feh = {
        ...
        exec = ''
          bash -c "f=%f; feh -. --start-at ./\\$(realpath --relative-to=\\$(dirname \\$f) \\$f)"'';
        terminal = false;
        type = "Application";
  ...
  in {
    ...
    mimeApps = {
      ...
      defaultApplications = let
        auto = mapAttrsToList
          (key: value: genAttrs value.mimeType (name: "${key}.desktop"))
          desktopEntries;
      in mkMerge
      (auto ++ [{ "application/x-terminal-emulator" = "kitty.desktop"; }]);
    };
  };
}
